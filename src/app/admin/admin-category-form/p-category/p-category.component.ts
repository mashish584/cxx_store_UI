...
export class ParentCategory implements OnInit,OnDestroy{
    ...
    constructor(private productService:ProductService){
        ...
        // by pushing new data
        this.subscription = productService.p_categoryUpdate.subscribe((data:Category) => {
    ...
    private addCategory(){
       ...
       let data:Category = {name:this.name,type:this.type,childs:[{}],parent:""};
       // sending a request to productService save category method
       // for saving category on server
       this.productService.storeProductCategory(data,true)
           .subscribe(
                (data:any) => {
